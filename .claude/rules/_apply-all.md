...
- PLAN MODE — When Claude Code's native plan mode is active, you MUST invoke the `give-plan` skill first before producing any plan. Plan mode provides the workflow structure (propose → approve → execute); `give-plan` shapes _how_ to plan (ground in code, pick the right altitude, surface risks, right-size the artifact, stop-and-wait).
