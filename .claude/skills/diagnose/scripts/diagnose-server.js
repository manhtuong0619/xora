...
const server = createServer((req, res) => {
  ...
  req.on("end", () => {
    try {
      ...
      res.end(
        JSON.stringify({ ok: true, sessionId: requestSessionId, logFile }),
      );
    } catch {
...
server.listen(PORT, HOST, () => {
  console.log(
    JSON.stringify({
      port: PORT,
      defaultSessionId: startupSessionId,
      defaultLogFile: logFileFor(startupSessionId),
    }),
  );
});
