...
If already in native plan mode, this skill shapes _how_ to plan; plan mode provides the workflow structure and must follow this skill guidelines.
...
- **Prose, not code** — describe _what_ changes in prose (name the file, the concept); never include executable syntax (function bodies, JSX, SQL, migration scripts, shell commands). "Illustrative" snippets are still implementation code.
