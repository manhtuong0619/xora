...
UI implementation: _"Extract component hierarchy, layout type, exact hex colors, typography (sizes/weights), spacing in px, interactive states, icons and decorative elements"_

Chart data: _"Extract chart type, axes with units, every data point with exact values, legend entries with colors. Output as a markdown table"_

Design review: _"Compare this screenshot against the design. Flag differences in spacing, colors, alignment, missing elements, and visual inconsistencies. Note exact values for each discrepancy"_
