...
The agent that just authored the code carries the narrative of _what it was trying to do_. That narrative biases judgment. A teammate with zero prior context evaluates the code on its merits, not on the intent behind it.
...
- **Intent** — the _what_, not the _why_. "Adds SSO via OIDC" is fine. "I chose OIDC because SAML felt too complex" is not.
